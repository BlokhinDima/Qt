...
TCPclient::TCPclient(QObject *parent) : QObject(parent)
{
    ...
    connect(socket, &QTcpSocket::connected, this, &TCPclient::SendSuccessConnStatus);
    connect(socket, &QTcpSocket::errorOccurred, this, &TCPclient::SendFailedConnStatus);
...
{   
    ...
    switch (header.idData)
    {
        ...
        case GET_STAT:
        {
            ...
            stream >> stat_server.incBytes
                   >> stat_server.sendBytes
                   >> stat_server.revPck
                   >> stat_server.sendPck
                   >> stat_server.workTime
                   >> stat_server.clients;
            emit sig_sendStat(stat_server);
            break;
        }
        case SET_DATA:
        {
            ...
            stream >> server_reply;
            emit sig_SendReplyForSetData(server_reply);
            break;
        }
        case CLEAR_DATA:
        {
...
void TCPclient::SendSuccessConnStatus()
{
    emit sig_connectStatus(StatusMessages::STATUS_SUCCES);
}


void TCPclient::SendFailedConnStatus()
{
